...
public class OrderExpeditionView extends VerticalLayout {
    ...
    private void setupGrid() {
        ...
        grid.addColumn(prep -> prep.getOrder().getAcceptsPartialExpedition() ? "SI" : "NO").setHeader("Acepta expedición parcial").setAutoWidth(true).setResizable(true);
        grid.addColumn(Preparation::getId).setHeader("Preparación").setAutoWidth(true).setResizable(true);
        grid.addColumn(Preparation::getStartingDate).setHeader("Inicio preparación").setAutoWidth(true).setResizable(true);
        grid.addColumn(Preparation::getEndingDate).setHeader("Fin preparación").setAutoWidth(true).setResizable(true);
    }
    ...
    private void createConfirmDialog(Preparation preparation) {
        ...
        confirmDialog.setHeader("Confirmar cédula del cliente");

        TextField clientDocumentTxt = new TextField("Cédula identidad");
        ...
        confirmDialog.addConfirmListener(e -> {
            if (!clientDocumentTxt.getValue().equals(preparation.getOrder().getCustomer().getId()))
                Notification.show("ERROR: La cédula proporcionada no coincide con la cédula de quien realizó el pedido", 5000, Notification.Position.BOTTOM_CENTER);
            else {
                expeditionService.shipPreparation(preparation);
                grid.setItems(this.preparationService.getAllPreparationsNotFinished());
                confirmDialog.close();
                Notification.show("La preparación ha sido correctamente expedida", 5000, Notification.Position.BOTTOM_CENTER);
            }
        });
